      ...
      <footer className="relative border-t border-gray-200/50 glass-card py-10 px-4">
        <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="border-t border-gray-200/50 pt-4 text-center text-[11px] text-gray-400 space-y-1">
            ...
            <p>
              통신판매업 신고번호: {process.env.NEXT_PUBLIC_BIZ_SALES_NO || '(신고번호)'} | 주소: {process.env.NEXT_PUBLIC_BIZ_ADDRESS || '(사업장 주소)'}
            </p>
            <p>
              전화: {process.env.NEXT_PUBLIC_BIZ_PHONE || '(전화번호)'} | 이메일: {process.env.NEXT_PUBLIC_BIZ_EMAIL || '(이메일)'}
            </p>
            <p>호스팅 서비스: Vercel Inc. | 결제 서비스: 토스페이먼츠(주)</p>
          </div>
        </div>
      </footer>
    </div>
